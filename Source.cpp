...
int FindProduct(string X, string Y)
{
	...
	if (m - 1 <= result) return 1;
	else return 0;
}
bool displayProductNeedFind(const vector <Product>& l, string x)
{
	if (l.empty())
	{
		cout << "\t\tPRODUCT LIST IS EMPTY" << endl;
...
void mainMenu(vector<Bill>& b, vector <Cart>& c, vector <Product>& l, Customer cus)
{
	...
	while (1)
	{
		...
		cout << "\t\t*" << setw(87) << "*\n";
		cout << "\t\t*   Enter (2) to looking for product" << setw(52) << "*\n";
		cout << "\t\t*" << setw(87) << "*\n";
		cout << "\t\t*   Enter (3) to view your cart" << setw(57) << "*\n";
		cout << "\t\t*" << setw(87) << "*\n";
		cout << "\t\t*   Enter (4) to pay your cart" << setw(58) << "*\n";
		cout << "\t\t*" << setw(87) << "*\n";
		...
		case 2://them hang vao gio :))))
		{
			...
			while (flag)
			{
				system("CLS");
				string x;
				cout << "Enter the product you are looking for" << endl;
				cin >> x;
				if (displayProductNeedFind(l, x))
				{
					addCartMenu(c, l);
					cout << "\t\tEnter (1) to buy another product or enter (0) to End ";
					cout << "\n\t\tInput your code: ";
